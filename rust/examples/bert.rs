...
async fn main() {
    ...
    let mut file_loader = FileParser::new();
    let files = file_loader.get_text_files(&PathBuf::from(r"D:\OneDrive\Downloads\Topmate"), None).unwrap();
    let mut _out = Vec::new();
    for file in files {
        let out = embed_file(file, &model, Some(&config), None::<fn(Vec<EmbedData>)>).await;
        match out {
            Ok(Some(out)) => _out.extend(out),
            _ => continue,
        }
    }
    let elapsed_time: std::time::Duration = now.elapsed();
    println!("Elapsed Time: {}. Number of chunks: {}", elapsed_time.as_secs_f32(), _out.len());

    let now = Instant::now();
    let _out = embed_directory_stream(
        PathBuf::from(r"D:\OneDrive\Downloads\Topmate"),
        &model,
        None,
    ...
    println!("Embedded files: {:?}", embedded_files_set.len());
