...
impl Idefics3ImageProcessor {
    ...
    pub fn resize_for_vision_encoder(
        ...
        let (new_height, new_width) = if width >= height {
            ...
            let height = ((height / vision_encoder_max_size).ceil() * vision_encoder_max_size).ceil();
            println!("Height: {:?}", height);
            (height, width)
        } else {
    ...
    pub fn resize(
        ...
        println!("Resized Image Dimensions: {:?}", (width, height));
    ...
    fn pad_image(
        ...
        let mut pixel_mask = ndarray::Array2::zeros((output_height as usize, output_width as usize));

        // Copy the original image into the padded image
        if data_format == "channels_first" {
            let transposed_image = image.to_owned().permuted_axes([2, 0, 1]);
            padded_image.slice_mut(s![.., ..input_height, ..input_width]).assign(&transposed_image);
            pixel_mask.slice_mut(s![..input_height, ..input_width]).fill(1);
        } else {
            padded_image.slice_mut(s![..input_height, ..input_width, ..]).assign(&image);
            pixel_mask.slice_mut(s![..input_height, ..input_width]).fill(1);
        }
    ...
    pub fn pad(
        ...
        let mut padded_masks = if return_pixel_mask {
            Some(vec![vec![ndarray::Array2::zeros((max_height, max_width)); max_num_images]; batch_size])
        } else {
            None
        };
        ...
        for (batch_idx, batch) in images.iter().enumerate() {
            for (sample_idx, image) in batch.iter().enumerate() {
                let (padded_image, pixel_mask) = self.pad_image(
                    image,
                    output_size,
                    constant_values,
                    data_format,
        ...
        (padded_images.into_iter().flatten().collect(), padded_masks.map(|masks| masks.into_iter().flatten().collect()))
    }

    fn _preprocess_one_image(&self, image: &DynamicImage) -> Result<(ndarray::Array4<f32>, Option<ndarray::Array3<i32>>), anyhow::Error> {
        ...
        let rescale_image_frames: Vec<ndarray::Array3<f32>> = if self.do_rescale {
            ...
            frames.iter().map(|frame| frame.map(|x| *x as f32)).collect()
        };

        // Step 5: Normalize frames
        let normalized_frames = if self.do_normalize {
            let image_mean = self.image_mean.clone().ok_or_else(|| anyhow::anyhow!("Missing image_mean"))?;
            let image_std = self.image_std.clone().ok_or_else(|| anyhow::anyhow!("Missing image_std"))?;
            ...
            rescale_image_frames.iter().map(|frame| {
        ...
        Ok((padded_images_concatenated, padded_masks_concatenated))
    }

    pub fn preprocess(&self, images: &[DynamicImage]) -> Result<(ndarray::Array5<f32>, Option<ndarray::Array4<i32>>), anyhow::Error> {
        let mut preprocessed_images = Vec::new();
        let mut preprocessed_masks = Vec::new();
        for image in images {
            let (padded_images, padded_masks) = self._preprocess_one_image(image)?;
        ...
        let preprocessed_images = ndarray::stack(ndarray::Axis(0), &image_views)?;
        ...
        Ok((preprocessed_images, preprocessed_masks))
    }
}
...
mod tests {
    ...
    fn image_resize_test() {
        ...
        let image_array = ndarray::Array3::from_shape_vec(
        ...
        let repo = api.repo(Repo::new(
            "HuggingFaceM4/Idefics3-8B-Llama3".to_string(),
            RepoType::Model,
        ));
        ...
        println!("{:?}", processor);
        let resized_image = processor.resize(&image, processor.size.clone().unwrap(), FilterType::Lanczos3);
        ...
        let rescale_image_frames = frames.iter().map(|frame| {
        ...
        let normalized_frames = rescale_image_frames.iter().map(|frame| {
            let frame = frame.to_owned();
            normalize(&frame, &image_mean, &image_std)
        }).collect::<Vec<_>>();
        
        // println!("Normalized Frames: {:?}", normalized_frames.len());
        let (padded_images, padded_masks) = processor.pad(&[normalized_frames], 0.0, true, "channels_first");
        let image_views: Vec<_> = padded_images.iter().map(|arr| arr.view()).collect();
        let padded_images_concatenated = ndarray::stack(ndarray::Axis(0), &image_views).unwrap();
        println!("Padded Images Concatenated: {:?}", padded_images_concatenated.shape());
