...
def test_adapter_upsert_call_file(
    ...
    assert (
        embed_anything.embed_file(
            test_pdf_file, embeder=bert_model, adapter=dummy_adapter
        )
        is None
    )
    assert (
        embed_anything.embed_file(
            test_txt_file, embeder=bert_model, adapter=dummy_adapter
        )
        is None
    )
...
def test_adapter_upsert_call_directory(bert_model, dummy_adapter, test_files_directory):
    assert (
        embed_anything.embed_directory(
            test_files_directory, embeder=bert_model, adapter=dummy_adapter
        )
        is None
    )
